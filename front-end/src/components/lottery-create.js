...
const LotteryCreate = () => {  
  ...
  let intervalStep = 100;
    ...
    function startLottery(){
      ...
      item.style.setProperty('transform', `rotateX(${rotateX}deg) translateZ(${translateZ}px)`);
    })
...
let rotate = () => {
  let winIndex = Math.round(rotateX / (360 / data.length) / data.length % 1 * data.length);
  if (tempDuration >= duration && checkWinner(winIndex)) {
    doneName.push(<li>{randomName}</li>)
    setDoneName(doneName);
    setRemainingNameData(remainingNameData.filter(name=>name!=randomName));
      clearInterval(interval);
              ...
              if (files) {
                Papa.parse(files[0], {
                  complete: function(results) {
                    ...
                    let nameList = convertedData.map((name) => <li className="list-item">{name}</li>);
                    setNameList(nameList);
                    document.getElementsByClassName('startLottery')[0].style.display="block"
                  }}
                )
  ...
  return <div>
      <input type="file" className ="csvInput" id="inputName" onChange={(e)=>{createList(e)}}></input>
      <label for="inputName" className="uploadButton">Upload list of name in CSV format</label>

      <button className="startLottery"onClick={startLottery}>Start Lottery</button>

      <div className="lottery-page">
          <div className="lottery-card">
            {randomName}
            <div className='remaining-name-collection' onClick={roll}>
            <ul className='rollscreen'>{nameList}</ul>
            </div>
            <button className ="rollButton" onClick={roll}>Tap to Roll</button>
            <div className='namelist'>
            <div><div>Done</div>
            <div className = 'completeList'><ul>{doneName}</ul></div></div>
            <div>
            <div>Remaining</div>
            <div className = 'remaininglist'><ul>{remainingNameData.map(x => <li>{x}</li>)}</ul></div>
            </div>
            </div>
